 ...
 * {@code ancestor}, {@code ancestor-or-self} and {@code attribute}. 
 * {@code preceding}, {@code precedingSibling}, {@code ancestor} and
 * {@code ancestor-or-self} traverse the DOM in reverse document order. For 
 * {@code ancestor} and {@code ancestor-or-self} this means we climb up the DOM.
 * The order for {@code attribute} is not guaranteed and the order should be
 * considered arbitrary. If at anytime during the traversal the callback 
 * function returns false, and only false (other values that evaluate to false, 
 * like null or '', won't affect the traversal), the guide will stop the
 * traversal at that node and return.
 */
var guide = {
    self: function(n, cb) {
        cb(n);
